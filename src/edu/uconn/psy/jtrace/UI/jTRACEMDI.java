...
//    private jTRACE [] jtChildren;           // like desktopPane.getAllFrames(), but no scriptingFrame
    ...
    static LinkedList globalPhonology;
    ...
    public jTRACEMDI() {
        ...
        initGalleryMenu();
        // import the phoneme sets based on .jt files in (root)/Phonology
        initPhonology();
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        jMenuItemCite.setMnemonic('c');
        jMenuItemCite.setText("Cite...");
        jMenuItemCite.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jMenuItemCiteActionPerformed(evt);
            }
        });

        menuHelp.add(jMenuItemCite);
    ...
    private void jMenuItemCiteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemCiteActionPerformed
        new CiteFrame().setVisible(true);
    }//GEN-LAST:event_jMenuItemCiteActionPerformed

    private void mdiKeyHandler(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_mdiKeyHandler
        // TODO add your handling code here:
        //System.out.println("key pressed in mdi frame: "+evt.getKeyChar()+"\t"+evt.getKeyModifiersText(evt.getModifiers()));
        char key = evt.getKeyChar();
        String modif = evt.getKeyModifiersText(evt.getModifiers());
        if((key=='s')&&(modif.startsWith("Ctrl")||modif.startsWith("Command"))){
            //menuItemSaveActionPerformed(evt);
        }        
        else if((key=='S')&&(modif.startsWith("Ctrl")||modif.startsWith("Command"))){
            //menuItemSaveAsActionPerformed(evt);
        }
        else if((key=='o'||key=='O')&&(modif.startsWith("Ctrl")||modif.startsWith("Command"))){
            //menuItemLoadActionPerformed(evt);
        }
    }//GEN-LAST:event_mdiKeyHandler
    ...
    private void initPhonology(){
        globalPhonology = new LinkedList();
                
        java.io.File phonologyDir = new java.io.File(edu.uconn.psy.jtrace.UI.traceProperties.rootPath.getAbsolutePath() +
                System.getProperty("file.separator") + "phonology");
        
        if(phonologyDir.exists()&&phonologyDir.isDirectory())
        {
            java.io.File[] phonologyItems = phonologyDir.listFiles();
            int incr = 0;
            for(int i=0;i<phonologyItems.length;i++){
                if(phonologyItems[i].getName().endsWith(".jt")||phonologyItems[i].getName().endsWith(".xml")){                    
                    try{
                    WTFileReader fileReader = new WTFileReader(phonologyItems[i]);
                    //load the phonology from the current file
                    TracePhones _tp = new TracePhones();
                    //add to the global list
                    globalPhonology.add(_tp);    
                    incr++;
                    } catch(Exception e){
                        e.printStackTrace();
                    }
    ...
    private void menuItemTileActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemTileActionPerformed
        ...
        int numFrames = desktopPane.getAllFramesInLayer(desktopPane.DEFAULT_LAYER.intValue()).length + desktopPane.getAllFramesInLayer(desktopPane.PALETTE_LAYER.intValue()).length;
...
//                } catch (java.beans.PropertyVetoException e) {}
    ...
    private void menuItemLoadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemLoadActionPerformed
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) 
        {
            ...
            traceProperties.workingPath = loadFile.getParentFile();
    ...
    private void exitHandler(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exitHandler
        if (tryToExit()){
            //save the working directory for next time:
            (new traceProperties()).storeWorkingDir();
            System.exit(0);
        }
    ...
    public static void main(String args[]) {
        ...
        if(args.length>0 && args[0].startsWith("-"))
            traceProperties.startupOptions=args[0];

        if(args!=null&&args.length>0&&(args[args.length-1].endsWith(".jt")||args[args.length-1].endsWith(".xml"))){        
            edu.uconn.psy.jtrace.UI.CommandLineUI ui = new edu.uconn.psy.jtrace.UI.CommandLineUI(args);
            ui.execute();                        
        }
        else{
            traceProperties.hasGUI=true;
    ...
    public void loadJTRACEFrame(TraceParam param, TraceSim sim, TraceSimAnalysis anal, GraphParameters gp, File file) 
    {
        ...
        if (openWindows < maxOpenWindows)
        {
            ...
            if(null != jFrame.getSim()){
                if(jFrame.getSim().getStepsRun()>0) {
                    jFrame.setFrameAll(jFrame.getSim().getStepsRun());
            ...
            //System.out.println("2.4 (TraceNet==null) : "+jFrame.getSim().isNullPointer()+"\n");            
        }
